    ...
    *,
    ...
    try:
        ...
        plan, step_usage = planner.make_plan(
            client, source, model=config.plan_model, style_guide=style_guide
        )
        record(step_usage)
        ...
        draft, step_usage = writer.write_draft(
            client, source, plan, model=config.write_model, style_guide=style_guide
        )
        record(step_usage)
        ...
        polished, step_usage = polisher.polish(
            client, draft, model=config.polish_model, style_guide=style_guide
        )
        record(step_usage)
...
def plan_only(
    ...
    *,
    ...
    try:
        on_step(1, "자료를 읽고 글의 각도와 목차를 잡는 중")
        plan, step_usage = planner.make_plan(
            client, source, model=config.plan_model, style_guide=config.style_guide
        )
        db.add_usage(
            conn, run_id, step_usage.input_tokens, step_usage.output_tokens, step_usage.cost_usd
        )
