...
**① 블로그로 만들 기사·글을 드래그해서 복사** (`⌘+C`)

**② 터미널에서 아래 두 줄을 차례로 실행**
...
첫 줄은 프로그램이 있는 폴더로 이동하는 것입니다. 터미널을 새로 켤 때마다 한 번 실행해야 합니다.
(터미널 맨 앞에 `Auto-writing-blog %` 라고 떠 있으면 이미 이동한 상태라 생략해도 됩니다.)

**③ 2~4분 기다리면 끝**

```
클립보드에서 7332자를 읽었습니다: "엔비디아의 Hugging Face 인수에 대한 생각..."
...
[완료] 초안이 만들어졌습니다.
  제목    엔비디아, 허깅페이스 129억 달러 인수설의 배경
  태그    엔비디아, 허깅페이스, 오픈소스AI, M&A, 반독점

다른 제목 후보
  - 중립성을 포기한 오픈소스 허브, 엔비디아 품으로
  - 젠슨 황이 침묵하는 이유, 허깅페이스 인수 협상
```

**자료를 파일로 저장할 필요가 없습니다.** 복사만 해두면 됩니다.

> 첫 줄에 `클립보드에서 …자를 읽었습니다` 가 뜨는지 꼭 확인하세요.
> 글자 수가 이상하면 복사가 제대로 안 된 것이니 다시 복사하고 실행하면 됩니다.

### 출처 링크를 남기고 싶다면

```bash
uv run blog write --source "https://원문주소"
```

이렇게 하면 글 끝에 출처가 자동으로 붙습니다. 안 붙여도 됩니다.

### 다른 방법 (필요할 때만)

| 명령 | 언제 |
|---|---|
| `uv run blog write` | **기본.** 복사해 둔 내용으로 글을 씁니다 |
| `uv run blog write --file 자료.txt` | 자료를 파일로 갖고 있을 때 |
| `uv run blog write --text "짧은 메모"` | 짧은 내용을 직접 칠 때 |
